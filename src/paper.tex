...
初步处理后，我们将特征图分为global branch和local branch，送入Stage Scan Structure(SSS)模块中。SSS模块由多个Stage组成，每个Stage包含多个Bi-WKV块。

\textit{1. Multi-Granularity Windows.}

特征图进入encoder后，我们对其进行分块，以便于进行特征提取。对于global branch与local branch，我们采用不同的分块方法。
global branch旨在捕捉长距离依赖，window size较大，block数量较少，序列短；local branch旨在捕捉局部细节，window size较小，block数量较多，
序列长\ref{fig:global branch和local branch}。这种不同Granularity的方法使得模型能够在不同层次上捕捉图像的全局与局部特征，提升了分割的精度。

可以看出，global batch的初始窗口分块可以表示为
\begin{equation}F_g \to g_{1}, g_{2}, \dots, g_{m}\end{equation}
其中m代表分块数量，$g_{i} \in \mathbb{R}^{w_g \times w_g \times C}$，$w_g$代表global branch的窗口行列。

同样地，local batch的初始窗口分块可以表示为\begin{equation}F_l \to l_{1}, l_{2}, \dots, l_{n}\end{equation}其中n代表分块数量，
$ l_{i} \in \mathbb{R}^{w_l \times w_l \times C}$，$w_l$代表local branch的窗口行列。

local branch的窗口大小小于global branch的窗口大小，即$w_l < w_g$，可以表示为\begin{equation}w_l = \frac{w_g}{k}\end{equation}
其中$k$为$g_{m}$相对于$l_{n}$的缩放系数，通常取2或4。在本文中，缩放系数被选择为2。

\textit{2. Stage-wise Window Mechanism.}

在SSS模块中，我们设计了Stage-wise Window Mechanism，根据不同Stage的编码器动态地调整global branch与local branch的窗口大小与分块数量。

我们设计SWM的理由是，随着网络层数加深（$\text{Stage} \in \{1, 4\}$），特征图的$H$与$W$逐渐减小，分辨率降低，语义信息增强，此时需要更多地关注全局上下文信息；
...
以层数stage为变量。由于缩放系数$k=2$，设全局块的空间尺寸在所有 stage 上固定为$w_g \times w_g, \quad w_g = 8.$
局部块的空间尺寸为$w_l \times w_l, \quad w_l = 4.$。则在第 $i\in \{1, 4\}$ 个 stage 上，全局块和局部块分别将特征图$F_s$划分为
\begin{equation}
    G_s = 
\end{equation}
\begin{equation}w_{g}^{i} = \frac{H}{2^{i} \cdot S_g}, \quad w_{l}^{i} = \frac{H}{2^{i} \cdot S_l}\end{equation}
其中 $H$ 是输入图像的高度（宽度相同）。随着 $i$ 的增加，$w_{g}^{i}$ 和 $w_{l}^{i}$ 逐渐减小，反映了特征图尺寸的变化。
