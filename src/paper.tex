...
同样地，llocal batch的初始窗口分块可以表示为\begin{equation}F_l \to l_{1}, l_{2}, \dots, l_{n}\end{equation}其中n代表分块数量，$ l_{i} \in \mathbb{R}^{w_l \times w_l \times C}$，$w_l$代表local branch的窗口大小。

local branch的窗口大小小于global branch的窗口大小，即$w_l < w_g$，可以表示为\begin{equation}w_l = \frac{w_g}{k}\end{equation}
其中$k$为缩放系数，通常取2或4。在本文中，缩放系数被选择为4。

\textit{2. Stage-wise Window Mechanism.}
因为在深层，特征图尺寸变小，语义信息更丰富，此时更需要模型具备理解全局上下文的能力。
随着网络加深（Stage $1 \to 4$），分辨率降低，全局分支逐渐退化为全图注意力，局部分支的感受野相对增大。
https://chatgpt.com/share/e/69429443-6fb0-800b-9848-bf9ae52816b7
Zig RiR的两次扫描和为一个BiWKV内进行，计算复杂度为
$$O=(2TC)$$
