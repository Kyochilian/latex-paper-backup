...
然而，传统的sweep scan会破坏空间连续性，导致贪吃蛇效应~\ref{fig:贪吃蛇效应}。在扫描过程中，BI-WKV只能看到扫描路径上的token，无法在二维空间感知
其他token的信息，也就破坏了网络的空间建模与特征提取能力。

ZigZag~\cite{zigrir}扫描方式虽然通过连续的S字形扫描缓解了空间连续性问题，但对于最重要的，无法看见扫描路径外的token，贪吃蛇效应依然存在。
为此，我们提出了两种解决方案：1、对角线扫描；2、Dynamic Q-shift。
对角线扫描的优势在于，能够同时捕捉水平与垂直方向的信息，进一步缓解空间连续性\ref{fig:各种扫描方式对比的图像，包括传统，S字，对角线}。
实验表明，在医学图像分割任务中，对角线扫描优于传统的扫描方式以及S字形扫描
