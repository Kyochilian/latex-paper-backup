...
stem模块的卷积核大小$k=3$,步长$s=2$，经过stem模块后，初始特征图$F_{0} \in \mathbb{R}^{\frac{H}{4} \times \frac{W}{4} \times C_{0}}$，其中通道数经过模块扩展为$C_{0}=64$。
可以看出，该模块的总下采样率 $S=4$。

对于global branch与local branch，我们采用不同的序列化方法。
