...
\subsection{Stem与预处理}
在图像输入的过程中，stem模块用于网络的起始部分，来进行预处理，为后续的编码器提供良好的初始特征~\cite{evitunet}。图像首先会经过一个3x3的卷积块，经过一个batch normalization(BN)，随后进入
...
以二维图像为例，我们将输入图像进行分片处理。
直接使用原始像素pixel-level会导致序列过长，提高计算度和低频噪声的出现~\cite{vit}，因此要对输入的图片进行预处理。我们利用卷机的局部归纳偏置，高效地提取初步特征。

\subsection{Stage Scan Structure}
因为在深层，特征图尺寸变小，语义信息更丰富，此时更需要模型具备理解全局上下文的能力。

可以在encoder的过程中，将特征图分解为block，层次越深，block越大，层次越浅，block越多、越小。扫描机制可以借鉴ZigRiR。
