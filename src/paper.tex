...
其中$\kappa$为$g_{m}$相对于$l_{n}$的缩放系数，通常取2或4。在本文中，缩放系数被选择为2。
...
以层数stage为变量。由于缩放系数$\kappa=2$，设全局块的空间尺寸在所有 stage 上固定为$w_g \times w_g, \quad w_g = 8.$
...
\begin{equation}
    F_i \xrightarrow{\text{Win}(\cdot; w_g)} g_{i,1}, g_{i,2}, \dots, g_{i,L_i^g}, \\
    \quad L_i^g \triangleq L_i(w_g), \quad g_{i,t} \in \mathbb{R}^{w_g \times w_g \times C_i};
\end{equation}
Local Branch可以表示为
\begin{equation}
    F_i \xrightarrow{\text{Win}(\cdot; w_l)} \ell_{i,1}, \ell_{i,2}, \dots, \ell_{i,L_i^\ell}, \quad L_i^\ell \triangleq L_i(w_l), \quad \ell_{i,t} \in \mathbb{R}^{w_l \times w_l \times C_i}.
\end{equation}
