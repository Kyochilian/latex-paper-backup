...
由式(33)的 Hilbert 双射 $\pi^{(i,w)}_{\mathrm{H}}(t)=(r_t,c_t)$，我们可以为每个序号 $t$ 定义其在扫描路径上的“切向”方向（forward tangent）为
\begin{equation}
\mathbf{d}_t =
\begin{cases}
...
接着，令四个单位方向为
\begin{equation}\label{eq:paper-auto-13}
...
\begin{equation}\label{eq:paper-auto-14}
a_{t,\delta}=|\mathbf{d}_t^\top \mathbf{e}_{\delta}|\in\{0,1\},
\end{equation}
其中 $a_{t,\delta}=1$ 表示方向 $\delta$ 与扫描切向平行（位于扫描路径上，包含 forward/backward 两向），
...
\begin{equation}\label{eq:paper-auto-15}
...
而对扫描法向施加更大的位移权重 $\alpha_{\perp}$，从机制上增强二维近邻的信息交换，
从而进一步缓解贪吃蛇效应并提升边界分割精度。

需要强调的是，$\alpha_{\perp},\alpha_{\parallel}$ 不引入额外的注意力计算，仅产生常数级的逐点乘法开销；
并且由于其仅由扫描顺序决定，在给定 $i,w$ 后可预计算，因此对推理速度影响极小。
...
在Channel Mix 中，我们遵循 Vision RWKV 的结构首先进入 Dynamic Q-shift。随后的$R_c$ 与 $V_c$ 分别经过 Sigmoid 以及 Squared ReLU，
