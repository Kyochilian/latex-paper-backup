...
$$\mathbf{W} \sim \mathcal{N}\left(0, \sqrt{\frac{2}{d_{in} + d_{out}}}\right)$$
...
$$\frac{\partial L}{\partial W_{ij}} = \sum_{n} \frac{\partial L}{\partial y_{nj}} \cdot \frac{\partial y_{nj}}{\partial W_{ij}} = \sum_{n} \frac{\partial L}{\partial y_{nj}} \cdot x_{ni}$$
...
$$\frac{\partial L}{\partial b_j} = \sum_{n} \frac{\partial L}{\partial y_{nj}} \cdot \frac{\partial y_{nj}}{\partial b_j} = \sum_{n} \frac{\partial L}{\partial y_{nj}}$$

\textbf{对输入的梯度（用于继续反向传播）：}
$$\frac{\partial L}{\partial x_{ni}} = \sum_{j} \frac{\partial L}{\partial y_{nj}} \cdot \frac{\partial y_{nj}}{\partial x_{ni}} = \sum_{j} \frac{\partial L}{\partial y_{nj}} \cdot W_{ij}$$
...
卷积运算的定义为：
$$\mathbf{Y}[n, c_o, h, w] = \sum_{c_i=0}^{C_{in}-1} \sum_{i=0}^{k_h-1} \sum_{j=0}^{k_w-1} \mathbf{X}[n, c_i, h \cdot s + i, w \cdot s + j] \cdot \mathbf{K}[c_o, c_i, i, j] + b[c_o]$$

其中 $s$ 为步长 stride。输出特征图的空间尺寸计算公式：
$$H_{out} = \left\lfloor \frac{H_{in} + 2 \cdot p - k_h}{s} \right\rfloor + 1, \quad W_{out} = \left\lfloor \frac{W_{in} + 2 \cdot p - k_w}{s} \right\rfloor + 1$$

\subsubsection{He 初始化 \cite{he_init}}
对于使用 ReLU 激活函数的网络，采用 He 初始化：
$$\mathbf{K} \sim \mathcal{N}\left(0, \sqrt{\frac{2}{C_{in} \times k_h \times k_w}}\right)$$
...
$$\frac{\partial L}{\partial K[c_o, c_i, i, j]} = \sum_n \sum_h \sum_w \frac{\partial L}{\partial Y[n, c_o, h, w]} \cdot X_{pad}[n, c_i, h \cdot s + i, w \cdot s + j]$$

\textbf{2. 对偏置的梯度 $\frac{\partial L}{\partial b}$：}
$$\frac{\partial L}{\partial b[c_o]} = \sum_n \sum_h \sum_w \frac{\partial L}{\partial Y[n, c_o, h, w]}$$
...
$$\frac{\partial L}{\partial X[n, c_i, h_i, w_i]} = \sum_{c_o} \sum_{h_o, w_o} \frac{\partial L}{\partial Y[n, c_o, h_o, w_o]} \cdot K[c_o, c_i, k_i, k_j]$$
...
\textbf{对 $\gamma$ 的梯度：}
$$\frac{\partial L}{\partial \gamma_c} = \sum_{n,h,w} \frac{\partial L}{\partial y_{n,c,h,w}} \cdot \hat{x}_{n,c,h,w}$$
...
$$\frac{\partial L}{\partial x} = \frac{\partial L}{\partial \hat{x}} \cdot \frac{\partial \hat{x}}{\partial x} + \frac{\partial L}{\partial \hat{x}} \cdot \frac{\partial \hat{x}}{\partial \mu} \cdot \frac{\partial \mu}{\partial x} + \frac{\partial L}{\partial \hat{x}} \cdot \frac{\partial \hat{x}}{\partial \sigma} \cdot \frac{\partial \sigma}{\partial x}$$

最终化简结果：
$$\frac{\partial L}{\partial x_{n,c,h,w}} = \frac{1}{m \cdot \sigma_c} \left( m \cdot \frac{\partial L}{\partial \hat{x}_{n,c,h,w}} - \sum_{i} \frac{\partial L}{\partial \hat{x}_i} - \hat{x}_{n,c,h,w} \cdot \sum_{i} \frac{\partial L}{\partial \hat{x}_i} \cdot \hat{x}_i \right)$$
...
$$\mu_{running} = (1-\alpha) \cdot \mu_{running} + \alpha \cdot \mu_{batch}$$
...
ReLU 的导数非常简单：
$$\frac{\partial \text{ReLU}(x)}{\partial x} = \begin{cases} 1 & \text{if } x > 0 \\ 0 & \text{if } x \leq 0 \end{cases} = \mathbf{1}_{[x>0]}$$

根据链式法则，反向传播时：
$$\frac{\partial L}{\partial x} = \frac{\partial L}{\partial y} \cdot \frac{\partial y}{\partial x} = \frac{\partial L}{\partial y} \cdot \mathbf{1}_{[x>0]}$$
...
推导过程：
$$\frac{d\sigma}{dx} = \frac{d}{dx}\left(\frac{1}{1+e^{-x}}\right) = \frac{e^{-x}}{(1+e^{-x})^2} = \frac{1}{1+e^{-x}} \cdot \frac{e^{-x}}{1+e^{-x}} = \sigma(x) \cdot (1-\sigma(x))$$

根据链式法则：
$$\frac{\partial L}{\partial x} = \frac{\partial L}{\partial y} \cdot \sigma(x) \cdot (1 - \sigma(x))$$
...
$$z_c = \frac{1}{H \times W} \sum_{h=1}^{H} \sum_{w=1}^{W} x_{c,h,w}$$
...
用学习到的权重对原始特征进行逐通道加权：
$$\tilde{x}_{c,h,w} = s_c \cdot x_{c,h,w}$$
...
$$\frac{\partial L}{\partial x_{c,h,w}}\bigg|_{mul} = \frac{\partial L}{\partial y_{c,h,w}} \cdot s_c$$

\textbf{对缩放因子 $\mathbf{s}$ 的梯度：}
$$\frac{\partial L}{\partial s_c} = \sum_{h,w} \frac{\partial L}{\partial y_{c,h,w}} \cdot x_{c,h,w}$$

然后通过 Sigmoid、FC2、ReLU、FC1 反向传播得到 $\frac{\partial L}{\partial \mathbf{z}}$。

\textbf{通过 GlobalAvgPool 对 $\mathbf{x}$ 的梯度：}
$$\frac{\partial L}{\partial x_{c,h,w}}\bigg|_{gap} = \frac{1}{H \times W} \cdot \frac{\partial L}{\partial z_c}$$

\textbf{最终梯度（两条路径相加）：}
$$\frac{\partial L}{\partial x_{c,h,w}} = \frac{\partial L}{\partial x_{c,h,w}}\bigg|_{mul} + \frac{\partial L}{\partial x_{c,h,w}}\bigg|_{gap}$$
...
$$\mathbf{y} = \underbrace{\text{Conv}_{1\times1} \rightarrow \text{BN} \rightarrow \text{ReLU} \rightarrow \text{Conv}_{3\times3} \rightarrow \text{BN} \rightarrow \text{ReLU} \rightarrow \text{Conv}_{1\times1} \rightarrow \text{BN}}_{\mathcal{F}(\mathbf{x})} + \mathbf{x}$$
...
$$p_i = \text{Softmax}(\mathbf{z})_i = \frac{e^{z_i}}{\sum_{j=1}^{C} e^{z_j}}$$

\subsubsection{交叉熵损失}
给定真实类别标签 $y$，交叉熵损失为：
$$L = -\log(p_y) = -z_y + \log\left(\sum_{j=1}^{C} e^{z_j}\right)$$

对于批量数据，损失取平均：
$$L = \frac{1}{N}\sum_{n=1}^{N} \left(-z_{n,y_n} + \log\sum_{j=1}^{C} e^{z_{n,j}}\right)$$

\subsubsection{反向传播的推导}
联合 Softmax 和交叉熵的梯度有一个非常简洁的形式 \cite{deeplearning}：
$$\frac{\partial L}{\partial z_i} = p_i - \mathbf{1}_{[i=y]}$$

即：对于正确类别，梯度为 $p_y - 1$（负值，推动增大）；对于错误类别，梯度为 $p_i$（正值，推动减小）。

\textbf{推导过程：}

$$\frac{\partial L}{\partial z_i} = \frac{\partial (-z_y + \log\sum_j e^{z_j})}{\partial z_i}$$
...
$$y_{c,h_o,w_o} = \max_{(i,j) \in R_{h_o,w_o}} x_{c, h_o \cdot s + i, w_o \cdot s + j}$$
...
$$\frac{\partial L}{\partial x_{c,h,w}} = \begin{cases} \frac{\partial L}{\partial y_{c,h_o,w_o}} & \text{if } (h,w) = \arg\max \\ 0 & \text{otherwise} \end{cases}$$
...
$$\frac{\partial L}{\partial x_{c,h,w}} = \frac{1}{H \times W} \cdot \frac{\partial L}{\partial y_c}$$
...
$$\frac{\partial L}{\partial \mathbf{x}} = \frac{\partial L}{\partial \mathbf{y}_n} \cdot \frac{\partial \mathbf{y}_n}{\partial \mathbf{y}_{n-1}} \cdot \ldots \cdot \frac{\partial \mathbf{y}_1}{\partial \mathbf{x}}$$
...
$$\frac{\partial L}{\partial \mathbf{x}} = \frac{\partial L}{\partial \mathbf{y}} \cdot \left( \frac{\partial \mathcal{F}}{\partial \mathbf{x}} + \mathbf{I} \right) = \frac{\partial L}{\partial \mathbf{y}} \cdot \frac{\partial \mathcal{F}}{\partial \mathbf{x}} + \frac{\partial L}{\partial \mathbf{y}}$$
