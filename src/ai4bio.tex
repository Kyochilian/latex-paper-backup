...
为弥补这一缺口，一些研究[cite]开始系统引入空间坐标、组织形态和跨切片关系。与非空间组学整合不同的是，******************，其中*****方法*********************************；这些方法充分利用了
空间结构建模，但也存在明显的局限性，它们依赖单一组学，信息上限有限，难以利用多组学信息（ RNA + ADT 或 RNA + ATAC ）的互补性。

为了进一步充分利用不同组学之间的数据与空间信息，实现更精准的空间组织分析，空间多组学整合方法开始被探索，并取得了显著进展。例如，
***等人提出的SpatialGlue采用..............；而SpaMICS则在此基础上。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。；SpaBalance实现了动态权重，并提出了跨组学共享学习的方法，

尽管空间多组学整合网络被不断提出，但目前，它们仍然存在一些局限性。首先，现有方法大多使用 fixed fusion，进行简单的加权和连接，缺少 spot-specific / instance-wise routing；
跨模态整合仍然更接近静态/粗粒度融合，对模态互补、模态冲突与不平衡聚类稳定性处理得不够显式。
多组学之间存在丰富的生物学信息表示，跨组学信号既有 consistent information，也有 complementary information，多数方法
把多组学 latent 表示融合到同一 shared embedding中；这类简单的拼接或加权可能会混淆两类信息。



我们的方法在三个重要方面不同于现有空间多组学方法。
首先，不同于大多数先前方法采用的固定融合方案，我们通过模态感知的混合专家模块进行逐节点自适应跨组学融合，允许不同 spot 根据其共识与差异模式由不同专家机制整合。
其次，我们不仅依赖一阶邻域图，还用 motif 增强图丰富了组学内拓扑建模，并将其与基于 Transformer 的全局上下文建模相结合。
